...
const EditPropertyPage = () => {
  ...
  useEffect(() => {
    if (user === null) {
      toast.error("Please log in to edit a listing", { id: "auth-required-edit" });
      router.push("/login");
      return;
    }
  ...
  const loadProperty = async () => {
    try {
      ...
      if (String(property.creator_id) !== String(user.id)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await propertyApi.update(params.id, formData);
    ...
    return (
      <main className="min-h-screen bg-white">
        <div className="flex min-h-[70vh] flex-col items-center justify-center px-4 text-center">
          ...
          <p className="mt-2 text-gray-500">This property may have been removed.</p>
          <button
